...
El sistema se desarrolla en Python 3.11+ con NumPy, sin recurrir a librerías de optimización VRP externas (OR-Tools, PyVRP, VRPy). Cada componente reside en un módulo independiente y testable de \pyid{src/memetico\_cvrp/}:
...
Cada \emph{run} se serializa con metadatos completos: \pyid{git\_commit\_hash}, versión de Python y librerías, hash MD5 de la instancia y \emph{timestamp} UTC. El script \pyid{scripts/verificar\_reproducibilidad.py} re-ejecuta los runs en modo \emph{smoke} y compara contra los resultados anteriores. Dos ejecuciones del mismo \emph{seed} sobre la misma máquina producen resultados idénticos bit a bit.
